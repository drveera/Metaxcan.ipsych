...
OPTS=`getopt -o h -l out:,gwas:,nojob,pop:,test,cov:,db:,man,csv:,labelthreshold:,tpop: -n 'metaxcan' -- "$@"`
...
done < $wd/weights.list${tpop} > $out/$out.adispatch
